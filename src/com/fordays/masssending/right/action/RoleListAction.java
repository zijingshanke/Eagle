package com.fordays.masssending.right.action;

import java.util.*;
import javax.servlet.http.*;
import org.apache.struts.action.*;
import com.fordays.masssending.right.RoleForm;
import com.fordays.masssending.right.RoleListForm;
import com.fordays.masssending.right.RoleRightForm;
...
public class RoleListAction extends Action {
	...
	public void doSysList(RoleListForm ulf, HttpServletRequest request) {
		...
		for (int i = 1; i < rowcount + 1; i++) {
			...
			rf
					.setRoleSystem((sdb.getColValue(i, "role_system").equals(
							"0") ? "true" : "false"));
			list.add(rf);
		}
	 ...
	 * String sql = ""; String userRoleID = ""; String sysRoleID = ""; String
	 * temp = ""; int rowcount = 0; int srowcount = 0; SelectDataBean sdb = new
	 ...
	 * System.out.println("sql=" + sql); sdb.setQuerySQL(sql);
	 * sdb.executeQuery(); rowcount = sdb.getRowCount(); ; for (int i = 1; i <
	 * rowcount + 1; i++) { RoleForm uf = new RoleForm(); temp =
	 * sdb.getColValue(i, "role_id"); // System.out.println("temp i=" + i + ", " +
	 * temp); if (i == 1) userRoleID = temp; uf.setRoleID(temp);
	 ...
	 * role_system=0"; // System.out.println("sql=" + sql);
	 * sdb.setQuerySQL(sql); sdb.executeQuery(); rowcount = sdb.getRowCount(); ;
	 * for (int i = 1; i < rowcount + 1; i++) { RoleForm uf = new RoleForm();
	 * temp = sdb.getColValue(i, "role_id"); if (i == 1) sysRoleID = temp;
	 ...
	 * ssdb.setQuerySQL(sql); ssdb.executeQuery(); srowcount =
	 * ssdb.getRowCount();
	 * 
	 ...
	 * String[srowcount]; else selectedRightItems = new String[0]; for (int j =
	 * 1; j < srowcount + 1; j++) { selectedRightItems[j - 1] =
	 * ssdb.getColValue(j, "right_key"); //
	 * System.out.println("selectedRightItems["
	...
	public void doUpdateUser4role(RoleListForm ulf, HttpServletRequest request,
		...
		if (dbt.isSuccess()) {
			inf.setMessage("鎮ㄥ凡缁忔垚鍔熺粰鐢ㄦ埛鎺堟潈!");
			inf.setForwardPage("/right/rolelist.do?thisAction=edituser4role");
		} else {
			inf.setMessage("鎿嶄綔澶辫触!" + dbt.getMessage());
			inf.setBack(true);
		}
	...
	public void doRight(RoleListForm ulf, HttpServletRequest request,
		...
		plb.setQuerySQL(sql.toString()); // 锟斤拷锟斤拷
		int rowcount = plb.getRowCount();
	...
	public void doDeleteRight(RoleListForm ulf, HttpServletRequest request) {
		...
		try {
			...
			System.out.println("删锟斤拷锟斤拷锟绞э拷埽锟�" + ex);
		}
	}

	public void doEditSysRight(RoleListForm ulf, HttpServletRequest request,
			int s) {
		String sql = "";
	...
	public void doEditUserRight(RoleListForm ulf, HttpServletRequest request,
		 ...
		 * sdb.executeQuery(); String[] sall = new String[sdb.getRowCount()];
		 * for (int i = 1; i < sdb.getRowCount() + 1; i++) sall[i - 1] =
		 * sdb.getColValue(i, "right_key");
		 * 
		 * sql = "select right_key from user_role_right where role_id=" +
		 * userRoleID; sdb.setQuerySQL(sql); sdb.executeQuery(); String[] s1 =
		 * new String[sdb.getRowCount()]; for (int i = 0; i < sdb.getRowCount();
		 * i++) s1[i - 1] = sdb.getColValue(i, "right_key");
		 */
	...
	private String doUpSelectedUserRight(RoleListForm ulf,
		...
		/*
		 * if
		 * (!ArrayUtil.isInclude(getRightKeyOfRole(uri.getRoleId().toString()),
		 * getRightKeyOfRole(userRoleID))) { Inform inf = new Inform();
		 * inf.setMessage("锟斤拷锟饺拷薏锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟饺拷锟斤拷锟斤拷锟饺拷锟�!");
		 * inf.setBack(true); request.setAttribute("inf", inf); return "inform"; }
		 */
