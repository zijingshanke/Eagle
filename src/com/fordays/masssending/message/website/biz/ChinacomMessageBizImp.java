...
/**
 * 中华网论坛 http://club.china.com/发帖接口实现类
 */
public class ChinacomMessageBizImp extends MessageUtil implements
		MessageUtilBiz {
	...
	public static void main(String[] args) {
		try {
			String LOGIN_SITE = "club.china.com";
			int LOGIN_PORT = 80;
			...
			PostMethod post = new PostMethod(
					"http://club.china.com/jsp/pub/xx.do?event=LOGIN&amp;1=1");
			post.setRequestHeader("Content-Type",
					"application/x-www-form-urlencoded;charset=UTF-8");

			NameValuePair url = new NameValuePair("url", "");// /jsp/pub/login.jsp

			NameValuePair username = new NameValuePair("username",
					"zijingshanke");
			NameValuePair password = new NameValuePair("password", "111111");
			NameValuePair cookietime = new NameValuePair("cookietime", "-1");
		...
		try {
			...
			if (200 == statusCode) {
				...
				messageInfo.setRemark("执行发帖后,statusCode:" + statusCode
						+ ",发帖失败");
				// HttpClientUtil.printResponseHtml(post, true);
